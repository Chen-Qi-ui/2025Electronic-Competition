...
{
	unsigned int ADCValue=0;
	ADC_TempSensorVrefintCmd(ENABLE);//使能内部温度传感器通道16和参考电压通道17
	ADC_RegularChannelConfig(ADC1,ADC_Channel_17, 1,ADC_SampleTime_239Cycles5);	//设置ADC1通道0的采样循序为1,采样时间为个周期
	ADC_SoftwareStartConvCmd(ADC1, ENABLE);										//ADC1的软件转换启动功能
	while(!ADC_GetFlagStatus(ADC1, ADC_FLAG_EOC));								//判断是否转换完成
	ADCValue = ADC_GetConversionValue(ADC1);
	return ADCValue;
...
********************************************************************/
#define N 200													//采样次数
#define BAN 50													//舍弃数据的个数
unsigned int GetADC1Average(unsigned char CH)  					//获得通道平均值
{ 
	unsigned short AdcValueBuf[N]={0},AdcValuetemp=0; 
...
kalman_filter ADC_kalman= {0.02,0.0,0.0,0.0,0.001,0.543};
kalman_filter ADC_CH17_kalman= {0.02,0.0,0.0,0.0,0.001,0.543};
