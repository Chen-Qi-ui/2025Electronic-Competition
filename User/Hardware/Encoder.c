...
void Encoder1_Init(void) {
    ...
    TIM_TimeBaseInitStructure.TIM_Period = 0xFFFF;  // ARR=65535
    ...
    TIM_SetAutoreload(TIM5, 0xFFFF);  // ??!??ARR????
    TIM_Cmd(TIM5, ENABLE);
}
...
int16_t Encoder1_Get(void)
{
	/*??Temp??????,?????CNT?????*/
	int16_t Temp;
	Temp = (int16_t)TIM_GetCounter(TIM5);
	TIM_SetCounter(TIM5, 0);
	return (int16_t)Temp;
}

//unsigned int Encoder_Get2(void)  //PWM2
//{

//unsigned int now = TIM_GetCounter(TIM2);
//unsigned int delta = now - last_count2;
//last_count2 = now;
//	return delta;
//}
